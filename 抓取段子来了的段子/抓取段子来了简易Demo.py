...
def sendmail(totalstr):
    datestr = datetime.strftime(datetime.now(), '(%m月/%d日更)')

    sendmail = SendMailClass()
    sendmail.sendmail(
                        ["3029068348@qq.com","891508172@qq.com"],
                        "<h1 style='color: red'>打败不开心"+datestr+"~~</h1>"+totalstr+"<p style='text-align: right;font-size: 3rem;color: #5db0fd'>——小海哥倾情奉献</p>",
                        emailimgArr=["http://cdn.iciba.com/news/word/big_20171014b.jpg","http://cdn.iciba.com/news/word/big_20171012b.jpg"],
                        emailsubject="打败不开心"+datestr,
...
#通过add_job来添加作业
sched.add_job(getnewestduanzi, 'interval', minutes=5)   #每隔5分钟发送一次
